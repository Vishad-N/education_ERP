...
|---|---|---|
| Application fee never completes | Pay creates a **Pending** fake order. Check status does not capture on the **deployed** image | Deploy capture-on-check (already in local `portal.py` / `App.vue`); then Razorpay Checkout + webhook |
...
- [ ] Deploy current `university_erp` (lead status New, handoff validate, fake payment check/confirm).
...
- [ ] Visible errors on upload, save, and pay (no silent fail).
- [ ] Staff actions as commands, not raw status fields: **Create application**, **Accept seat**, **Confirm admission**, **Create student**.
- [ ] Published form, year, program, seats, and fee policy created by school setup, not by a one-off script.
- [ ] Idempotency keys generated by the system; do not ask clerks to type them.
...
- [ ] Fee demand generation from published policy → submitted Sales Invoice → cashier and portal collection → receipt PDF.
...
- [ ] Offer accept/reject/expire from portal (guardian), not only Desk.
- [ ] Application status on the guardian page after submit (not “school will update”).
- [ ] Student portal access issued at conversion (SMS/email link, expiry, revoke).
- [ ] Hindi review by a native speaker; low-literacy pass on a cheap Android.
- [ ] Dynamic program/class from published offerings.
